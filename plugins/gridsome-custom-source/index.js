...
class WordPressSource {
  static defaultOptions () {
    return {
      baseUrl: 'https://www.acadomia.fr',
      apiBase: 'wp-json',
      perPage: 100,
      concurrent: 10,
      filterPostsBefore: false,
      typeName: 'WordPress',
      ...
      featuredImagesLocalPath: '', // Path to store featured images
    }
  }
    ...
    return map(fragments, (fragment, index) => {
      ...
      if (image) {       
        ...
        return {
          ...
          order: index+1,
          fragmentData: { 
            html: filterHtmlFragment(fragment)
          }
        }
      }
    })    
  ...
  async getPosts (store) {
    ...
    for (const type in this.restBases.posts) {
      ...
      var postsUrl =  `wp/v2/${restBase}`;
      if (this.options.filterPostsBefore) {
        postsUrl = postsUrl + `&before=${this.options.filterPostsBefore}`
      }

      const data = await this.fetchPaged(postsUrl)
...
/**
 * Apply filters to html fragments
 */
function filterHtmlFragment(html) {
  // const regex = /<iframe.*?\/iframe>/gm
  // let m
  ...
  //         html = html.replace(match, `<div class="video-container">${match}</div>`)
  //     }, this);
  // }
  return html
}
